...
async function clickResiliently(page: any, element: any, selectorDescription: string) {
  try {
    // Try to scroll the element into view first so actionability is easier to pass, with very short timeout
    await element.scrollIntoViewIfNeeded({ timeout: 1000 }).catch(() => {});
    
    // Focus first to prepare the element
    await element.focus({ timeout: 1000 }).catch(() => {});
    
    // Attempt normal click with a short timeout of 2 seconds
    await element.click({ timeout: 2000 });
  } catch (err) {
    addLog("warning", `Standard click failed on ${selectorDescription}: ${(err as Error).message}. Trying instant dispatchEvent fallback...`);
    try {
      // dispatchEvent is extremely reliable and instant, bypasses all visibility and actionability checks
      await element.dispatchEvent("click");
    } catch (err2) {
      addLog("warning", `dispatchEvent click failed: ${(err2 as Error).message}. Trying JS evaluate click fallback...`);
      try {
...
async function runRealPostingWithPage(page: any, url: string, message: string): Promise<{ status: "success" | "expired" | "captcha" | "failed" | "retry"; message: string }> {
  try {
    ...
    addLog("error", `Playwright page posting execution failed: ${(error as Error).message}`);
    return { status: "failed", message: (error as Error).message };
  }
}
...
async function executeFetchTrending(): Promise<Coin[]> {
  addLog("info", "Starting cryptocurrency listings fetch (Prioritizing CoinMarketCap Trending 50)...");
  botStatus = "Fetching";
  ...
  // STAGE 1: Try CoinMarketCap Public Data API for Trending/Most Visited (Fast & API Keyless)
  try {
    addLog("info", "Stage 1: Attempting CoinMarketCap Public Data API (trending/most-visited)...");
    const response = await fetch("https://api.coinmarketcap.com/data-api/v3/cryptocurrency/trending/most-visited?limit=50&timeRange=24h", {
      headers: {
    ...
    if (response.ok) {
      ...
      const list = resData?.data?.cryptoCurrencyList || resData?.data || [];
      if (Array.isArray(list) && list.length > 0) {
        coins = list.map((coin: any, index: number) => {
          const usdQuote = coin.quotes?.[0] || coin.quote?.[0] || {};
          return {
            name: coin.name || "",
            symbol: (coin.symbol || "").toUpperCase(),
            price: parseFloat(usdQuote.price || coin.price || "0"),
            change_24h: parseFloat(usdQuote.percentChange24h || coin.percentChange24h || "0"),
            change_1h: parseFloat(usdQuote.percentChange1h || coin.percentChange1h || "0"),
            change_7d: parseFloat(usdQuote.percentChange7d || coin.percentChange7d || "0"),
            market_cap: parseFloat(usdQuote.marketCap || coin.marketCap || "0"),
            volume_24h: parseFloat(usdQuote.volume24h || coin.volume24h || "0"),
            cmc_rank: coin.cmcRank || coin.rank || (index + 1),
            slug: coin.slug || "",
            url: `https://coinmarketcap.com/currencies/${coin.slug || ""}/`,
          };
        }).filter(c => c.name && c.symbol && c.slug);

        if (coins.length > 0) {
          successSource = "CoinMarketCap Public Data API (Trending)";
        }
      }
    } else {
      addLog("warning", `Stage 1 Data API returned non-200 status: ${response.status}`);
    }
  } catch (err) {
  ...
  if (coins.length === 0 && process.env.CMC_API_KEY) {
    try {
      addLog("info", "Stage 3: Attempting fetch from CoinMarketCap Pro API (listings/latest)...");
      ...
      if (response.ok) {
        ...
        if (resData.data && Array.isArray(resData.data)) {
          ...
          successSource = `CoinMarketCap Pro API (Listings, Credits: ${creditCount})`;
        }
      } else {
        addLog("warning", `Stage 3 Pro API returned error status: ${response.status}`);
      }
    } catch (err) {
      addLog("warning", `Stage 3: CoinMarketCap Pro API failed: ${(err as Error).message}`);
    }
  }

  // STAGE 4: CoinGecko Public Markets API
  if (coins.length === 0) {
    try {
      addLog("info", "Stage 4: Attempting fetch from CoinGecko Public Markets API...");
      const response = await fetch("https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=50&page=1&sparkline=false&price_change_percentage=1h,24h,7d");
      ...
      if (response.ok) {
        ...
        if (Array.isArray(cgData) && cgData.length > 0) {
          ...
          successSource = "CoinGecko Markets API";
        }
      } else {
        addLog("warning", `Stage 4 CoinGecko returned error status: ${response.status}`);
      }
    } catch (err) {
      addLog("warning", `Stage 4: CoinGecko API failed: ${(err as Error).message}`);
    }
  }

  // STAGE 5: Load local cache from previous successful fetching runs
  if (coins.length === 0) {
    try {
      addLog("info", "Stage 5: Attempting to load from local cached trending coins...");
      const cached = readJsonFile<Coin[]>(LAST_TRENDING_FILE, []);
      if (cached && cached.length > 0) {
        coins = cached;
        successSource = "Local Cache File (last_trending.json)";
      }
    } catch (err) {
      addLog("warning", `Stage 5: Loading from cache failed: ${(err as Error).message}`);
    }
  }

  // STAGE 6: High-fidelity simulated real-time mock data
  if (coins.length === 0) {
    addLog("warning", "Stage 6 Fallback triggered. Creating high-fidelity simulated trending coin list...");
    const mockCoinsRaw = [
    ...
    successSource = "Dynamic High-Fidelity Simulated Market Source";
  }

  // Save the result to local disk so it persists and matches expectations
  writeJsonFile(LAST_TRENDING_FILE, coins);
  addLog("success", `Successfully loaded ${coins.length} trending coins from ${successSource}.`);
  botStatus = "Idle";
  return coins;
}
...
async function runPostingLoop() {
  ...
  if (runMode === "Real Browser" && messages.length > 0) {
    addLog("info", "Starting persistent Playwright browser instance for optimized batch posting...");
    try {
  ...
  try {
    while (isPostingRunning && currentPostingIndex < messages.length) {
      ...
      // Wait 1-2 seconds to simulate human timing before navigating
      ...
      if (runMode === "Real Browser" && page) {
        addLog("info", `Executing optimized page posting context for ${item.name}...`);
        try {
          const realResult = await runRealPostingWithPage(page, item.url, item.message);
          outcome = realResult.status;
          messageText = realResult.message;
        } catch (err) {
          outcome = "failed";
          messageText = `Playwright page error: ${(err as Error).message}`;
          addLog("error", `Error during automation of ${item.symbol}: ${messageText}`);
        }
      } else {
        // Simulation mode
    ...
    if (browser) {
      addLog("info", "Closing persistent Playwright browser instance...");
      await browser.close().catch(() => {});
    }
  }
